...
async def main(loop):
    reader, _ = await serial_asyncio.open_serial_connection(url=args["readPort"], baudrate=115200)
    print('Reader created')
    _, writer = await serial_asyncio.open_serial_connection(url=args["writePort"], baudrate=115200)
